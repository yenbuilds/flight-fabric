...
test('a scored takeoff renders every fact as text with a tone', () => {
  ...
  assert.equal(card.scoreText, 'Runway use 95%');
  ...
  assert.equal(card.climb.rotationDetailText, 'Steady rotation');
  assert.equal(card.climb.maxPitchText, '+12.1 deg');
  assert.equal(card.alignment.lateralText, 'ON CL');
  assert.equal(card.alignment.lateralGradeText, 'Perfect');
  assert.equal(card.alignment.lateralGradeTone, 'text-green-400');
  assert.equal(card.alignment.headingText, '1.2 deg R');
  assert.equal(card.alignment.headingGradeText, 'Runway aligned');
...
test('an overrun keeps the runway-end facts explicit and never reads as praise', () => {
  ...
  assert.equal(card.climb.screenTone, 'text-red-400');
  ...
  assert.equal(card.alignment.lateralGradeText, 'Good', 'the lateral fact itself remains visible in the metrics');
});
...
test('missing geometry is reported as unavailable, not as a good takeoff', () => {
  ...
  assert.equal(card.climb.screenText, 'Not reached');
  ...
  assert.match(card.debrief.confidenceReason, /Scored after a timeout/);
...
test('settle-backs, late liftoff and rapid rotation escalate their tiles', () => {
  ...
  assert.equal(card.climb.rotationDetailText, 'Rapid rotation');
...
test('the Overview preview derives from the same card state', () => {
  ...
  assert.match(pending.status, /Scoring the climb-out/);
