...
export function createQrSvgData(text, options = {}) {
  ...
  const quietZone = Number.isInteger(options.quietZone) ? Math.max(0, options.quietZone) : 2;
  const errorCorrectLevel = options.errorCorrectLevel || QRErrorCorrectLevel.M;
  const qr = new QRCode(0, errorCorrectLevel);
  qr.addData(value);
  qr.make();
