<script>
function formatMicrosoft737Max8Draft(rawValue) {
  const current = Number(rawValue);
  if (!Number.isFinite(current)) return '';
  return Number.isInteger(current) ? String(current) : String(Number(current.toFixed(2)));
}

export function reconcileMicrosoft737Max8NumericDraftState(state = {}, snapshot = {}, previous = []) {
  const {
    rawValue,
    unavailable = false,
    profileKey,
  ...
  if (!next.dirty) next.draft = formatMicrosoft737Max8Draft(rawValue);
  return next;
}
</script>
...
const selectorControls = Object.freeze([
  {
    id: 'speed', label: 'SPD', fieldId: 'mcp.speedKts', actionId: 'flightGuidance.speed.set',
    groupId: 'flightGuidance.speed', min: 100, max: 399, step: 1, unit: 'kt', inputmode: 'numeric',
  },
  {
    id: 'heading', label: 'HDG', fieldId: 'mcp.headingDeg', actionId: 'flightGuidance.heading.set',
    groupId: 'flightGuidance.heading', min: 0, max: 359, step: 1, unit: 'deg', inputmode: 'numeric',
  },
  {
    id: 'altitude', label: 'ALT', fieldId: 'mcp.altitudeFt', actionId: 'flightGuidance.altitude.set',
    groupId: 'flightGuidance.altitude', min: 0, max: 49000, step: 100, unit: 'ft', inputmode: 'numeric',
  },
  {
    id: 'vertical-speed', label: 'V/S', fieldId: 'mcp.verticalSpeedFpm', actionId: 'flightGuidance.verticalSpeed.set',
...
const modeControls = Object.freeze([
  { id: 'ap-master', label: 'GENERIC AP MASTER', fieldId: 'afds.apMaster', prefix: 'flightGuidance.apMaster' },
  { id: 'flight-director', label: 'FD', fieldId: 'afds.flightDirector', prefix: 'flightGuidance.flightDirector' },
  { id: 'autothrottle-arm', label: 'A/T ARM', fieldId: 'afds.autothrottleArmed', prefix: 'flightGuidance.autothrottleArmed' },
  { id: 'speed', label: 'SPD', fieldId: 'afds.speed', prefix: 'flightGuidance.speedHold' },
  { id: 'heading', label: 'HDG', fieldId: 'afds.headingSelect', prefix: 'flightGuidance.headingHold' },
  { id: 'altitude', label: 'ALT', fieldId: 'afds.altitudeHold', prefix: 'flightGuidance.altitudeHold' },
  { id: 'vertical-speed', label: 'V/S', fieldId: 'afds.verticalSpeed', prefix: 'flightGuidance.verticalSpeedHold' },
  { id: 'nav', label: 'NAV', fieldId: 'afds.lnav', prefix: 'flightGuidance.navHold' },
  { id: 'approach', label: 'APPR', fieldId: 'afds.approach', prefix: 'flightGuidance.approachHold' },
  { id: 'flight-level-change', label: 'FLC', fieldId: 'afds.levelChange', prefix: 'flightGuidance.flightLevelChange' },
...
for (const control of selectorControls) {
  watch(
    ...
    ([rawValue, unavailable, profileKey, sourceStatus, pending], previous = []) => {
      const next = reconcileMicrosoft737Max8NumericDraftState({
...
function controlStatusId(control) {
  return `microsoft-737-max-8-status-${control.groupId.replace(/[^A-Za-z0-9_-]/g, '-')}`;
}
...
function numericStatusId(control) {
  return `microsoft-737-max-8-target-status-${control.id}`;
}
...
const pageStatus = computed(() => {
  ...
  const actionableStates = [
    ...
    ...[...modeControls, ...lightControls, ...surfaceControls, flapControl]
      .flatMap((control) => actionIdsFor(control).map((actionId) => (
        actionReadbackAvailable(control)
        && actionSupported(actionId)
      ))),
  ];
  if (!actionableStates.every(Boolean)) {
    return actionableStates.some(Boolean)
...
<template>
  <div class="space-y-5 p-3 sm:p-4" data-aircraft-template="microsoft-737-max-8" :data-aircraft-profile-key="profileKey">
    <header class="flex flex-wrap items-start justify-between gap-3">
      <div class="min-w-0">
        <h3 class="text-base font-semibold text-gray-100">Microsoft / Asobo Studio Boeing 737 MAX 8</h3>
        <p class="mt-0.5 text-xs leading-relaxed text-gray-500">Compact MCP, standard guidance, exterior-light and flight-configuration controls with live readback.</p>
      </div>
    ...
    <p class="rounded-md border border-surface-200 bg-surface-50 px-3 py-2 text-[10px] leading-relaxed text-gray-400" aria-live="polite">
      {{ pageStatus }} AP MASTER and mode commands are generic simulator controls; they do not represent Boeing CMD A/B channel selection. A/T active, runway-turnoff, speedbrake and deep systems remain read-only.
    </p>

    <section data-microsoft-737-max-8-section="mcp">
      ...
      <div class="grid grid-cols-1 gap-2 sm:grid-cols-2 xl:grid-cols-4">
        <form
          ...
          :data-microsoft-737-max-8-selector="control.id"
          :data-aircraft-control-group="control.groupId"
          @submit.prevent="submitNumeric(control)"
        >
          <div class="flex items-center justify-between gap-3">
            <label class="text-[9px] font-semibold uppercase tracking-widest text-gray-500" :for="`microsoft-737-max-8-target-${control.id}`">{{ control.label }}</label>
          ...
          <div class="mt-2 flex gap-2">
            <input
              :id="`microsoft-737-max-8-target-${control.id}`"
              type="text"
      ...
      <div class="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-3 xl:grid-cols-5">
        <div
          ...
          :key="control.id"
          class="rounded-lg border border-surface-200 bg-surface-50 p-3"
          :data-microsoft-737-max-8-mode="control.id"
          :data-aircraft-control-group="control.groupId"
        >
          <div class="flex items-center justify-between gap-3">
      ...
      <div class="mt-2 grid grid-cols-1 gap-2">
        <div class="rounded border border-surface-200 bg-surface-50 px-2.5 py-2 text-[10px] text-gray-300" data-aircraft-field="afds.autothrottleActive">
          A/T ACTIVE <span class="float-right font-mono font-semibold">{{ booleanText('afds.autothrottleActive') }}</span>
          <div class="mt-1 text-[9px] text-gray-500">Read only</div>
        </div>
      </div>
    </section>

    <section data-microsoft-737-max-8-section="exterior-lights">
      <div class="dashboard-section-kicker">Exterior Lights</div>
      <p class="mb-2 text-[10px] leading-relaxed text-gray-500">OFF and ON send fixed commands. Lamp output is readback, not cockpit-selector confirmation.</p>
      <div class="grid grid-cols-1 gap-2 sm:grid-cols-2 xl:grid-cols-4">
        <div
          v-for="control in lightControls"
          :key="control.id"
          class="rounded-lg border border-surface-200 bg-surface-50 p-3"
          :data-microsoft-737-max-8-light="control.id"
          :data-aircraft-field="control.fieldId"
          :data-aircraft-control-group="control.groupId"
        >
          <div class="flex items-center justify-between gap-3">
            <span class="text-[9px] font-semibold uppercase tracking-widest text-gray-500">{{ control.label }}</span>
        ...
        <div class="rounded-lg border border-surface-200 bg-surface-50 p-3" data-microsoft-737-max-8-light="runway-turnoff-readonly" data-aircraft-field="lights.runwayTurnoff">
          <div class="flex items-center justify-between gap-3">
    ...
    <section data-microsoft-737-max-8-section="configuration">
      ...
      <div class="grid grid-cols-1 gap-2 sm:grid-cols-3">
        <div
          ...
          :key="control.id"
          class="rounded-lg border border-surface-200 bg-surface-50 p-3"
          :data-microsoft-737-max-8-surface="control.id"
          :data-aircraft-control-group="control.groupId"
        >
          <div class="flex items-center justify-between gap-3">
            <span class="text-[9px] font-semibold uppercase tracking-widest text-gray-500">{{ control.label }}</span>
        ...
        <div class="rounded-lg border border-surface-200 bg-surface-50 p-3" data-microsoft-737-max-8-surface="flaps" :data-aircraft-control-group="flapControl.groupId">
          <div class="flex items-center justify-between gap-3">
      ...
      <div class="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-3">
        ...
        <div class="rounded border border-surface-200 bg-surface-50 px-2.5 py-2 text-[10px] text-gray-300 sm:col-span-3" data-aircraft-field="controls.speedbrakePercent">SPEEDBRAKE <span class="float-right font-semibold">{{ decimalText('controls.speedbrakePercent') }}%</span><div class="mt-1 text-[9px] text-gray-500">Read only</div></div>
      </div>
    </section>

    <section data-microsoft-737-max-8-section="systems">
      <div class="dashboard-section-kicker">Two-Engine &amp; System Snapshot</div>
      <p class="mb-2 text-[10px] leading-relaxed text-gray-500">Engine, fuel and pressurization data are concise read-only telemetry; no deep-system writes are exposed.</p>
      <div class="grid grid-cols-2 gap-2">
        <div v-for="engine in [1, 2]" :key="engine" class="rounded-lg border border-surface-200 bg-surface-50 p-3" :data-microsoft-737-max-8-engine="engine">
