...
This folder contains bundled cabin-announcement packs. Users can also add audio
in their application-data folder.

## How playback works

Backend telemetry drives cabin announcements:
...
- The frontend maps the phase and slot to a filename, finds it, queues it, and
  plays it through `HTMLAudioElement`.
...
These slots and filenames are supported:
...
The bundled `standard` pack contains:
...
record its source, licence, and required attribution in
`THIRD_PARTY_NOTICES.md`. Do not copy audio from airlines, simulators, stock
libraries, or other sources without that review.
...
If the phase changes before its dwell ends, the pending announcement is canceled.
...
- During startup and while on the ground, altitude samples establish a baseline
  without audio. This prevents an old crossing from replaying.
...
The frontend then falls back to bundled files under:
...
This overrides the bundled `standard/pushback-start.wav`.
...
Use saved settings for normal configuration.

## Playback controls

Cabin announcements have no dedicated header indicator or mute button.
