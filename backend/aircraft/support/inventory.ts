...
function buildReport({ profile, integration, catalogue, sourceProfiles, generatedAt = new Date().toISOString() }: Record<string, any>) {
  const commands = [...catalogue.inventory].sort((a, b) => a.id.localeCompare(b.id));
  const fields = Object.values<any>(integration?.fields || {}).sort((a, b) => a.id.localeCompare(b.id));
  const actions = Object.values<any>(integration?.actions || {}).sort((a, b) => a.id.localeCompare(b.id))
...
function observationDependencies(value, result = new Set<string>()) {
...
function diffReports(before, after) {
  ...
  const changes: Record<string, { added: string[]; removed: string[]; changed: string[] }> = {};
  for (const name of ['fields', 'actions', 'commands']) {
    const oldItems = new Map<string, any>(before.contract[name].map(item => [item.id, item]));
    const newItems = new Map<string, any>(after.contract[name].map(item => [item.id, item]));
    changes[name] = {
...
export { profileKey, buildReport, buildValidationPlan, diffReports, renderReport, assertReport, fingerprint };
