...
function logbookOutcomeGrade(entry: Partial<LandingEntry>): string | null {
  ...
  return touchdownSeverity > vsSeverity ? touchdownGrade : (vsGrade || touchdownGrade);
}
...
function extractEntry(payload: GenericRecord | null | undefined): LandingEntry | null {
  ...
  return {
    ...
    aircraftProfileId: payload.aircraft_profile_id || null,
    ...
    runwayExcursion: toBool(payload.runway_excursion) ?? false,
...
function computeGroupTrendRows(
  ...
  return [...groups.entries()]
    .map(([key, groupEntries]) => {
      ...
      const stableValues = groupEntries.filter((entry) => typeof entry.gateStable === 'boolean');
      const stableCount = stableValues.filter((entry) => entry.gateStable === true).length;
      ...
      return {
        ...
        stableRatePct: stableValues.length > 0 ? Math.round((stableCount / stableValues.length) * 100) : null,
...
function parseLandingsFromContent(
  ...
  gradeLanding: (vsFpm: number) => GenericRecord,
  ...
  for (let index = 1; index < lines.length; index += 1) {
    ...
    if (values[recordTypeIndex] !== 'LANDING') continue;

    const vsFpm = typeof row.vs_fpm === 'number' ? row.vs_fpm : null;
    if (vsFpm === null || vsFpm >= 0) continue;
    const storedGrade = typeof row.grade === 'string' && row.grade.trim() ? row.grade.trim() : null;
    const gradeResult = storedGrade ? null : gradeLanding(vsFpm);
    const grade = storedGrade || (gradeResult ? gradeResult.grade : null);
    ...
    landings.push({
      ...
      aircraftProfileId:
        typeof row.aircraft_profile_id === 'string' && row.aircraft_profile_id.trim()
      ...
      gateStable: stability.gateStable,
      bounceCount: typeof row.bounce_count === 'number' ? row.bounce_count : null,
      bounceGrade: typeof row.bounce_grade === 'string' && row.bounce_grade.trim() ? row.bounce_grade.trim() : null,
      runwayExcursion: toBool(row.runway_excursion) ?? false,
...
async function getLandingsFromCsvFile(filePath: string, options: CsvFileReadOptions = {}): Promise<LandingEntry[]> {
  ...
  const { gradeLanding } = require('./landing.js') as {
    gradeLanding: (vsFpm: number) => GenericRecord;
  };
  ...
  try {
    ...
    if (!options.bypassCache && canReuseLandingCache(cached, filePath, mtimeMs)) {
    ...
    const fileLandings = parseLandingsFromContent(content, filePath, parseCsvLine, gradeLanding, splitCsvLines);
